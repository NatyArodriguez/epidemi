...
from epidemi.core.temp_model import anual_temp, TemperatureGenerator, TemperatureGeneratorWithNoise, TemperatureGeneratorWithNoiseStep
...
data_normal = np.loadtxt('hystoric_temp_new.txt', skiprows=1)
data_gamma = load_data_file('gamma_decil.txt')

def temp_matrix_noisy(k,beta,deltas, n_iterations, season, suma, days, ci,
                              data_normal=data_normal, data_gamma=data_gamma):
    """It generates a matrix where the number of columns is the number of\
        iterations desired and the number of rows is equal to the days of\
        a non-leap year. Each element represents the number of total cases\
        generated by a single imported case which entered the day of the date\
    ...
    temp_gen = TemperatureGeneratorWithNoiseStep()
    ...
    for j in range(n_iterations):
        ...
        tmin = np.tile(tmin_serie, 17)
        tmean = np.tile(tmean_serie, 17)
        aux = u.daily_cases(k, beta, season, suma, ci, rain, tmin,
                    tmean, hr)
        # aux[1]: new cases, aux[2]: v/h
        matriz[:,j] = aux[2]
    return matriz
